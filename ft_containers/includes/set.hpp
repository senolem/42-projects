...
/*   Updated: 2023/01/24 15:41:14 by albaur           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	...
	class set
	{
		...
		public:
			...
			set(InputIt first, InputIt last, const key_compare &comp = key_compare(), const allocator_type &alloc = allocator_type()) : _tree(comp, alloc), _key_compare(comp)
			{
			...
			{
				if (_tree.search(_tree.get_root(), key))
					return (1);
				else
					return (0);
			}
